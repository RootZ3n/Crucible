...
type UiCtx = {
  normalizeRunForDisplay: (run: unknown) => {
    ...
    operationalRows: { tokensIn: number; tokensOut: number; cost: number; correctness: number }[];
    pass: unknown;
    model: string;
    provider: string;
  };
  selectModelBatch: (tabKey: string, kind: "local" | "cloud") => void;
  allModelIdsByKind: (kind: "local" | "cloud") => string[];
  setFocusedRun: (tabKey: string, run: unknown) => void;
  handleRun: (tabKey: string) => Promise<void>;
  state: {
...
function loadUi(): UiCtx {
  ...
  const ui = (sandbox as { __ui: UiCtx }).__ui;
