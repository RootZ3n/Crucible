...
describe("ui-layout-regression: control rail does not re-introduce the clipping combo", () => {
  it(".control-rail must NOT have max-height on viewport (root cause of the clipping bug)", () => {
    ...
    assert.doesNotMatch(
      body,
      /max-height\s*:\s*calc\(\s*100vh/,
      "Regression: .control-rail should not cap at 100vh; that was the clipping root cause. Use sticky positioning without max-height instead.",
    );
  });
  ...
  it("first rail panel must NOT have overflow:hidden (clips CONTROL DECK content)", () => {
    ...
    assert.doesNotMatch(
      body,
      /overflow\s*:\s*hidden/,
      "Regression: overflow:hidden on the first rail panel clips buttons/fields when the panel is flex-crushed",
    );
  });
...
describe("ui-layout-regression: benchmark lane — visible test picker replaces the dropdown", () => {
  it("no TASK single-select dropdown binding to selectedTask remains", () => {
    ...
    assert.doesNotMatch(
      ui,
      /<label>\s*TASK\s*<\/label>\s*<select[^>]*updateTabField\([^)]*selectedTask/,
      "Regression: the TASK single-select dropdown must be gone; test selection is now visible and toggleable.",
    );
  });
  ...
  it("renders a Live Status panel at the top of the content column, above the leaderboard", () => {
    ...
    assert.doesNotMatch(
      aside![0]!,
      /renderLiveStatus\(/,
      "Live Status must not live inside the control-rail aside anymore — it must be in the content column",
    );
  });
...
describe("ui-layout-regression: selection-to-execution integrity (count tri-state, per-test dispatch)", () => {
  ...
  it("handleRun uses selectedTasks + runCount (not a hidden spec default)", () => {
    ...
    assert.doesNotMatch(
      ui,
      /JSON\.stringify\s*\(\s*\{[^}]*task\s*:\s*['"]spec[- _]?\d+['"]/,
      "must not hardcode any spec task id into the run payload",
    );
  });
...
describe("ui-layout-regression: Providers tab is wired and backed by the registry API", () => {
  it("ships a 'Providers' tab in TAB_CONFIG and routes to renderProvidersView()", () => {
    ...
    assert.match(
      ui,
      /state\.activeTab\s*===\s*'providers'[\s\S]{0,60}renderProvidersView\(\)/,
      "renderActiveTab must dispatch providers → renderProvidersView()",
    );
  });
